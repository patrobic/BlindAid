...
Parse::Parse(Core::CoreParameters *params, Tools::Logger *logger) : Class(params, logger),
...
void Parse::Configure()
{
  ...
  if (CheckFlag("-depthdim", 2))
    _modes.SetDORegion(_prms);

  if (CheckFlag("-colordim", 2))
    _modes.SetTLRegion(_prms);
